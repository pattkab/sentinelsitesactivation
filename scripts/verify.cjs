...
(async()=>{
 ...
 assert.equal(await page.locator('.resource').count(),14);
 ...
 await page.locator('#clear-search').click();assert.equal(await page.locator('.resource').count(),14);
 await page.locator('#resource-category').selectOption('Reporting');assert.equal(await page.locator('.resource').count(),2);
 await page.locator('#resource-category').selectOption('');
 ...
 await browser.close();console.log('PASS: all 5 views, 14 downloads, resource search/filter/empty state, checklist persistence/reset, confidential roster removal, guide disclosure, 3 viewport widths and 200% font sizing. No JavaScript errors.');
